...
st.set_page_config(page_title='Gerenciador de Eventos', page_icon='🗓️', layout='wide')

API_URL = 'http://127.0.0.1:8000'

st.title('🗓️ GERENCIADOR DE EVENTOS')
st.markdown('---')

#Abas
tab1, tab2, tab3, tab4 = st.tabs(['➕ Novo Evento', '🗒️ Listagem', '📆 Calendário', '📩 Importar'])

#Tab1: Novo Evento
with tab1:
    st.header('Adicionar Novo Evento')

    with st.form('form_evento'): #Formulário
        nome = st.text_input('Nome do evento', placeholder='Ex: Reunião com cliente')
        data = st.date_input('Data')
        hora = st.time_input('Hora')

        if st.form_submit_button('💾 Salvar Evento', type = 'primary'): #criar botão
            if nome:
                data_hora = datetime.combine(data, hora).isoformat()
                try:
                    response = requests.post( #conversar com API no front
                        f'{API_URL}/eventos/',
        ...
        if response.status_code == 200:
            ...
            if not eventos:
                ...
                for evento in eventos_filtrados:
                    ...
                    status = '🕰️ PASSADO' if data_obj < agora else '⏳FUTURO'
                    ...
                    with st.expander(f"📌 {evento['nome']} - {data_fmt} {status}"):
                        ...
                        with col_btn1:
                            if st.button(f'📝 Editar', key = f"edit_{evento['id']}", use_container_width = True):
                            ...
                            if st.button(f'🗑️ Deletar', key = f"delete_{evento['id']}", use_container_width = True):
                                try:
                                    ...
                                    if del_response.status_code == 204:
                                        st.success(f'✔️ Evento deletado!')
                                        st.rerun()
                                    else:
...
if 'edit_id' in st.session_state:
    ...
    with st.form('form_editar'):
        ...
        data_edit = st.text_input('Data', value=st.session_state.edit_data)
        hora_edit = st.text_input('Hora', value=st.session_state.edit_hora)
            ...
            if st.form_submit_button('🔄 Atualizar', type='primary', use_container_width=True):
                ...
                try:
                    ...
                    if put_response.status_code == 200:
                        ...
                        st.error('✖️ Erro ao atualizar')
                except requests.exceptions.ConnectionError:
                    st.error('🛜 Erro de conexão')

        with col_btn2:
            if st.form_submit_button('❌ Calendar', use_container_width=True):
                del st.session_state.edit_id
                st.rerun()
...
with tab4:
    ...
    uploaded_file = st.file_uploader('Escolha um arquivo JSON', type = ['json']) #abre caixa de diálogo

    if uploaded_file is not None:
        #Carrega dados do arquivo JSON
        try:
            #retorna um bytesIO. Json_load pode ler diretamente
            dados_json = json.load(uploaded_file)

            st.subheader('🗒️Eventos a importar')
            ...
            if not dados_json:
                ...
                with st.expander('Visualizar eventos'):
                    for idx, evento in enumerate(dados_json, 1):
                        try:
                            #Converteer data_hora se estiver em formato string
                            ...
                            st.markdown(f'    🗓️ {data_fmt}')
                        except Exception as e:
                            st.warning(f"**{idx}. {evento.get('nome', 'Evento sem nome')}** - ⚠️ Erro ao processar evento")
                ...
                if st.button('📩 Importar eventos para o Banco de Dados', type='primary', use_container_width=True):
                    ...
                    with col_result3:
                        st.metric('🗒️ Total', len(dados_json))
...
st.markdown(
    ...
    "📆 Gerenciador de Eventos | Full-stack com SQLAlchemy + FastAPI + Streamlit"
    "</small></div>",
